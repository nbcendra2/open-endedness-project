...
def append_reflection(env_name: str, reflection_text: str, max_reflections: int = 3) -> None:
    with _reflection_lock:
        task_id = _task_id_from_env_name(env_name)
        path = _reflections_path(task_id)
        # Inline the read instead of calling load_reflections (which would deadlock)
        reflections = []
        if os.path.exists(path):
            try:
                ...
                if isinstance(data, list):
                    reflections = [str(x) for x in data[-max_reflections:]]
            except Exception:
                pass
...
""")
    ...
    def _generate_triggers_batch_llm(self, step_idx: int) -> None:
        ...
        env_type = "TextWorld-TreasureHunter" if (
            "treasure" in self._env_name.lower() or "tw-" in self._env_name.lower()
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    f"You are an expert {'text-adventure' if 'tw-' in self._env_name.lower() else 'BabyAI'} agent reflecting on a completed episode. "
